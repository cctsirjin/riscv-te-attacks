...
// Cache mapping for SiFive Freedom U740 (FU740-C000) SoC (S7 + U74 cores)
// Refer to https://sifive.cdn.prismic.io/sifive/1a82e600-1f93-4f41-b2d8-86ed8b16acba_fu740-c000-manual-v1p6.pdf
// Each U74 core’s private L1 instruction and data caches are configured to be a 4-way set-associative 32 KiB cache.
#define L1_ICACHE_WAYS 2 // Degree of associativity N = 2. i.e. 2-way set associative.
#define L1_ICACHE_BLOCK_BYTES 64 // b = 64Byte. "Fixed cache line length of 64 bytes."
#define L1_ICACHE_BLOCK_BITS 6 // = log2(b).
#define L1_ICACHE_SETS 256 // S = 256. S=B/N=C/Nb. Here C=32KiB, L1_ICACHE_WAYS=N=2, b=64B, so we have S=32KiB/(2x64B)=(1/4)K=256.
#define L1_ICACHE_SETS_BITS 8 // = log2(S)
#define L1_ICACHE_CAPACITY_BYTES (L1_ICACHE_SETS*L1_ICACHE_WAYS*L1_ICACHE_BLOCK_BYTES) // Cache Capacity C=Bb=SNb=0.5K*64Byte=32KiB
 ...
 */
#define OFFSET_MASK (~(FULL_MASK << L1_ICACHE_BLOCK_BITS)) // Offset bits are 0 to (L1_ICACHE_BLOCK_BITS - 1).
// After this definition, only offset bits remain 1, and they are used to discover existence of any cache set in use.
#define TAG_MASK (FULL_MASK << (L1_ICACHE_SETS_BITS + L1_ICACHE_BLOCK_BITS)) 
...
uint8_t dummyMem[MULTIPLIER * L1_ICACHE_CAPACITY_BYTES];
// Temporary variable.
...
void flushCache(uint64_t memAddr, uint64_t memSize){
    //printf("Flushed memAddr(0x%x) tag(0x%x) set(0x%x) off(0x%x) memSize(%d)\n", memAddr, (memAddr & TAG_MASK) >> (L1_ICACHE_SETS_BITS + L1_ICACHE_BLOCK_BITS), (memAddr & SET_MASK) >> L1_ICACHE_BLOCK_BITS, memAddr & OFFSET_MASK, memSize);

    // Find out the quantity of blocks that need to be cleared.
    uint64_t numSetsClear = memSize >> L1_ICACHE_BLOCK_BITS; // This operation removes block bits and shifts set bits to the right.
    ...
    if (numSetsClear > L1_ICACHE_SETS){
        numSetsClear = L1_ICACHE_SETS;
    }
    ...
    uint64_t alignedMem = (((uint64_t)&dummyMem) + L1_ICACHE_CAPACITY_BYTES) & TAG_MASK;
    ...
    for (uint64_t i = 0; i < numSetsClear; ++i){
        ...
        uint64_t setOffset = (((memAddr & SET_MASK) >> L1_ICACHE_BLOCK_BITS) + i) << L1_ICACHE_BLOCK_BITS;
        //printf("setOffset(0x%x)\n", setOffset);

        // There are N=L1_ICACHE_WAYS in a set to flush. And it needs to be repeated for 5 times since we have an extended dummyMem.
        for(uint64_t j = 0; j < ((MULTIPLIER-1)*L1_ICACHE_WAYS); ++j){
            // offset to reaccess the set
            uint64_t wayOffset = j << (L1_ICACHE_BLOCK_BITS + L1_ICACHE_SETS_BITS);
